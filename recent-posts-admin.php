...
function recent_posts_options_page(){
	...
	_e('help and instructions', 'recent_posts_plugin'); 
	echo '</a></h2></div>';
	if (!RecentPosts::check_post_plugin_library(__('<h1>Please install the <a href="http://downloads.wordpress.org/plugin/post-plugin-library.zip">Post Plugin Library</a> plugin.</h1>', 'recent_posts_plugin'))) return;
...
function recent_posts_placement_options_subpage(){
	...
	?>
		<div class="wrap">
		<h2><?php _e('Placement Settings', 'recent_posts_plugin'); ?></h2>
		<form method="post" action="">
		<div class="submit"><input type="submit" name="update_options" value="<?php _e('Save Placement Settings', 'recent_posts_plugin') ?>" /></div>
		<table class="optiontable form-table">
			<?php 
...
function recent_posts_install() {
	...
	if (!isset($options['feed_parameters'])) $options['feed_parameters'] = 'prefix=<strong>'.__('Recent Posts', 'recent_posts_plugin').':</strong><ul class="recent-posts">&suffix=</ul>';
	if (!isset($options['append_on'])) $options['append_on'] = 'false';
	if (!isset($options['append_priority'])) $options['append_priority'] = '10';
	if (!isset($options['append_parameters'])) $options['append_parameters'] = 'prefix=<h3>'.__('Recent Posts', 'recent_posts_plugin').':</h3><ul class="recent-posts">&suffix=</ul>';
